...
def login_with_password(data: LoginRequest, db: Session = Depends(get_db)):
    """
    Sprint 5 – Demo Mode:
    - Email znaleziony → wystawia token BEZ weryfikacji hasła.
    - Email nie znaleziony → auto-tworzy konto i wystawia token.
    """
    ...
    if not user:
        ...
        db.commit()
        db.refresh(user)
    # Demo mode: hasło jest całkowicie ignorowane dla istniejących kont.
